...
class DatabaseHelper {
  ...
  factory DatabaseHelper({required String baseUrl}) {
    final instance = DatabaseHelper._init();
    instance._syncService = DatabaseSyncService(
      baseUrl: baseUrl,
      dbHelper: instance,
    );
    return instance;
  ...
  Future<Database> _initDB(String filePath) async {
    ...
    bool serverHasData = await _checkServerData();
    print("serverHasData: $serverHasData");
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
          ...
          // Only insert default data if server has no data
          print("serverHasData: " + serverHasData.toString());

          if (!serverHasData) {
            print("No server data found. Inserting default gates, spaces, and vehicle types...");
  ...
  Future<bool> _checkServerData() async {
    try {
      print('Checking server data from: ${ApiConfig.initCheckUrl}');
      ...
      final response = await http.get(
        Uri.parse(ApiConfig.initCheckUrl),
        headers: headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        final hasData = data is Map && data.containsKey('hasData') ? data['hasData'] == true : false;
        print('Parsed hasData: $hasData');
        return hasData;
      } else {
        print('Unexpected server status code: ${response.statusCode}');
        return false;
      }
    } catch (e) {
      print('Error checking server data: $e');
      return false; // Consider retry logic if server data is critical
    }
  }
  ...
  // Modified insert entry method to include gate information
...
// Add to DatabaseHelper class
  Future<void> logSecurityEvent(String eventType, String timestamp) async {
    final db = await database;
  ...
  Future<void> clearAllDataOnLogout() async {
    try {
      ...
      final dbPath = await getDatabasesPath();
      final path = join(dbPath, 'parking.db'); // Match the filePath used in _initDB
      await databaseFactory.deleteDatabase(path);
      print('Database file deleted successfully.');

      // Optionally clear the token from SharedPreferences
      final prefs = await SharedPreferences.getInstance();
